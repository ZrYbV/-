...
def setup_chinese_font():
    """检测系统可用的中文字体并设置 matplotlib"""
    # 常见中文字体候选列表
    font_candidates = [
        'SimHei', 'Microsoft YaHei', 'SimSun', 'FangSong', 'KaiTi',
        'Noto Sans CJK SC', 'WenQuanYi Micro Hei', 'AR PL UMing CN', 'AR PL UKai CN'
    ]
...
def generate_astronomical_data(lat, lon, start_date, end_date, timezone_hours=8):
    ...
    while current_date <= end_date:
        ...
        try:
            ...
            sunrise_local = ephem.localtime(sunrise_utc) - timedelta(hours=timezone_hours)
            sunset_local = ephem.localtime(sunset_utc) - timedelta(hours=timezone_hours)
        ...
        try:
            ...
            moonrise_local = ephem.localtime(moonrise_utc) - timedelta(hours=timezone_hours)
            moonset_local = ephem.localtime(moonset_utc) - timedelta(hours=timezone_hours)
        ...
        try:
            ...
            dawn_local = ephem.localtime(dawn_utc) - timedelta(hours=timezone_hours)
            dusk_local = ephem.localtime(dusk_utc) - timedelta(hours=timezone_hours)
