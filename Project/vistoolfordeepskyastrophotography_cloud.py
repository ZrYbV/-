...
FONT_PATH = "/tmp/NotoSansSC-Regular.otf"  # 写入临时目录（云端可写）
...
import time
os.environ['TZ'] = 'UTC'  # 强制 ephem 以 UTC 为基准
time.tzset()
...
def calc_timezone_offset(lon_str):
    ...
    if direction == 'W':
        lon_value = -lon_value
    timezone = int(round(lon_value / 15.0))
    offset_timezone = 8 - timezone
...
def generate_astronomical_data(lat, lon, start_date, end_date, timezone_hours=8):
    ...
    while current_date <= end_date:
        ...
        try:
            ...
            sunrise_local = ephem.to_datetime(sunrise_utc).replace(tzinfo=None) + timedelta(hours=timezone_hours)
            sunset_local = ephem.to_datetime(sunset_utc).replace(tzinfo=None) + timedelta(hours=timezone_hours)
        ...
        try:
            ...
            moonrise_local = ephem.to_datetime(moonrise_utc).replace(tzinfo=None) + timedelta(hours=timezone_hours)
            moonset_local = ephem.to_datetime(moonset_utc).replace(tzinfo=None) + timedelta(hours=timezone_hours)
        ...
        try:
            ...
            dawn_local = ephem.to_datetime(dawn_utc).replace(tzinfo=None) + timedelta(hours=timezone_hours)
            dusk_local = ephem.to_datetime(dusk_utc).replace(tzinfo=None) + timedelta(hours=timezone_hours)
...
def compute_data(lat_str, lon_str, start_date_str, end_date_str, beijing_time):
    ...
    if beijing_time:
        ...
        offset_timezone = calc_timezone_offset(lon_str)
    target_tz = 8 - offset_timezone

    df = generate_astronomical_data(lat, lon, start_date, end_date, timezone_hours=offset_timezone)
    return df, lat, lon, start_date, end_date, total_days, offset_timezone, target_tz

# ================= 主绘图函数 =================
def create_figure(df, lat_str, lon_str, lat, lon, start_date, end_date, total_days, offset_timezone, target_tz):
    # 准备绘图数据
...
if st.sidebar.button("🚀 生成图表", type="primary"):
    ...
    with st.spinner("正在计算天文数据并生成图表，请稍候..."):
        # 计算数据（缓存）
        df, lat, lon, start_date, end_date, total_days, offset_timezone, target_tz = compute_data(
            lat_str, lon_str, start_date_str, end_date_str, beijing_time
        )
        # 生成图表
        fig = create_figure(df, lat_str, lon_str, lat, lon, start_date, end_date, total_days, offset_timezone, target_tz)
        st.pyplot(fig)
